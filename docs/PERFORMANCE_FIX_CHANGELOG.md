...
| 2026-09-08 | business/settings.py | BOARD-01 | `GZipMiddleware` added — dynamic HTML was going over the wire uncompressed (gunicorn is the edge on Railway; WhiteNoise only compresses static) | Low | `check`; `Content-Encoding: gzip` asserted on all three date pages | legs dashboard 12.2 MB → 380 KB, planner 1.68 MB → 146 KB, board 623 KB → 66 KB on a 191-leg day. BREACH: Django re-masks the CSRF token per response (verified 3 distinct tokens across 3 requests) |
| 2026-09-08 | dispatching/views.py, .../legs_filter.html | BOARD-02 | Legs dashboard driver dropdowns ship as stubs (placeholder + assigned driver) and clone the roster from one `<template>` on first interaction | Med | full `dispatching`+`reservations` suite (2275 tests); rendered stub label asserted byte-identical to its roster entry | was the full roster in every row, twice per leg: 372 selects / 23,129 `<option>` = 4.7 MB of the 12.2 MB page. Page 12.2 → 7.6 MB raw, 74,951 → 54,158 DOM nodes. Rollback = revert; nothing else reads these selects |
| 2026-09-08 | reservations/models.py | BOARD-03 | `Leg.controlling_flight` reads prefetched `legflight_set` rows when present instead of always issuing a filtered query | Low | suite; `Meta.ordering` deterministic + partial unique constraint ⇒ same row `.first()` picked | the filtered queryset could never hit a prefetch cache, so every leg on the day cost a round trip |
| 2026-09-08 | dispatching/views.py | BOARD-04 | Prefetch/select_related gaps: `driver__profile` on the board's legs, `legflight_set__flight` on the planner's legs and on all three prev-day queries | Low | query counts captured before/after per page | board 232 → 26 queries, planner 103 → 23, dashboard 39 → 29 |
