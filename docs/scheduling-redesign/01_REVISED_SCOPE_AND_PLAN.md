...
| D13 | **The Day Manager — a future phase, sequenced after this one.** Once the day-builder proves itself: a *live* tool that watches the board as the day unfolds and recommends/eventually makes real-time adjustments — moving trips, swapping drivers or vehicles, rearranging handoffs, farming out when necessary — in response to flight delays, drivers falling behind, vehicle outages, new bookings, cancellations. Explicitly not the builder's job to predict these; different clock, different tool. Do not design Build 3b around anticipating live changes — that overcomplicates it and isn't its purpose. |
| D14 | **Design for eventual auto-apply; do not build it now.** v1 stays propose-only (Ticket E) — this does not change. But nothing should be architected in a way that would make turning on automatic application difficult later, once trusted (promotion remains D11's call). Checked 2026-08-24: the current design already reuses the real write doors and enforces every hard-wall check before a plan is scored, both of which are exactly what future automation would need — no changes made. Two **pre-existing** gaps flagged as prerequisites for real automation (not blockers for Build 3b, both already scoped as v1 non-goals in 05 Ticket E): (1) `auto_assign_drivers`'s apply path re-implements `set_leg_driver` inline instead of calling the front door, and has drifted (missing attribution fields) — must be reconciled before an unattended write can be trusted; (2) `apply_day_setup` cannot delete a `DriverVehicleAssignment` row, so "leave this driver off" cannot self-apply. Park both as known future work. |
| D15 | **The two Gate-4 findings are accepted for v1 — diagnose later (2026-08-25).** (a) The shipped engine's own builds place 1–4 driver-sides/day under the 510-min rest floor vs actual adjacent-day work (rest is a soft penalty in placement, not a wall). Accepted for now; criterion 5 is read as "the plan adds NO rest breach beyond the same-date engine baseline" (which holds everywhere), the absolute number stays printed as evidence, and a possible engine hard-gate is a future ticket the founder may open after diagnosing. (b) On 2026-08-03 the engine baseline covers 81.7% vs the hand-finished board's 83.7% (humans fielded 16 driver-days vs the engine's 14) and pairing swaps don't close it. Accepted for now; criterion 1 passes when the plan matches state B *or* matches a baseline that itself falls short — the shortfall prints as an accepted finding. Neither ruling turns the feature on (that stays D11). |
| D16 | **Coverage beats idle capacity (2026-08-25): "I would rather have 100% in-house and give a driver a few jobs than farm a job."** When trips are farming out while certified drivers sit available and cars sit free, the Day-Builder PROPOSES ticking named bench drivers onto named free cars — each suggestion must capture at least one otherwise-farmed trip through a full-pipeline evaluation and pass the same walls (overnight rest both sides, availability window, Sprinter certification, no share conflicts), or it is not shown. Still propose-only: the dispatcher ticks and makes the call — nothing contacts a driver. This refines the Build-3b session's "no add-a-driver suggestions" delegation call; it does NOT revisit the CUT roster ladder (no size search — targeted, named, self-paying additions only). The fixed-headcount plan and Gate-4 criterion 9 are unchanged; the gate re-derives every addition claim (a false capture claim fails the gate). |
