...
that leg's pickup address**, which answers "how far out is he?" in one click. On
the Fleet pages there is no job in view, so the row becomes **Show on map** over
a plain pin on the coordinates.
...
| Not shown | Why |
|---|---|
| The destination name | You right-clicked that trip; the menu header already names it. "→ directions to Disney's Grand Floridian Resort & Spa" was the longest line in the menu, restating the thing you clicked. |
...
- **A one-ended route is refused.** `maps_directions_url` returns None unless
  ...
  with a blank pickup falls back to the pin.
