...
def sync_hubspot_view(request):
    ...
    sync_type = request.GET.get(
        "type", "reservations"
    )  # 'reservations', 'customers', or 'all'
...
def sync_customers(start_id=0, limit=None, dry_run=False):
    ...
    for customer in query:
        result_entry = {"customer_id": customer.id, "name": customer.get_full_name()}
...
def sync_reservations(start_id=0, limit=None, dry_run=False):
    ...
    query = (
        Reservation.objects.filter(id__gt=start_id)
    ...
    for reservation in query:
        result_entry = {
            ...
            "customer": reservation.customer.get_full_name()
                ...
                if hasattr(reservation, "payments"):
                    ...
                    if has_payments:
                        ...
                        payment_data = {
                            ...
                            if hasattr(latest_payment, "amount")
                            and latest_payment.amount
                            else None,
                        }
                    ...
                    if has_payments and payment_data:
                        try:
                            ...
                            payment_status = status_map.get(
                                latest_payment.status, "Unknown"
                            )

                            # Get payment method if available
                            payment_method = "N/A"
                            if (
                                hasattr(latest_payment, "customer")
                                and latest_payment.customer
                            ):
                                if (
                            ...
                            result_entry["payment_synced"] = payment_result.get(
                                "success", False
                            )
                            ...
                            logger.error(
                                f"Error syncing existing payment for reservation #{reservation.id}: {payment_error}"
                            )
