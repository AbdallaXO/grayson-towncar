...
class MockReservation:
    ...
    def save(self):
        print(
            f"Saving reservation {self.id} with hubspot_deal_id = {self.hubspot_deal_id}"
        )
...
def find_deal_by_reservation_id(reservation_id):
    ...
    if DRY_RUN:
        logger.info(
            f"DRY RUN: Would search for deal with reservation_id = {reservation_id}"
        )
        return None
...
def sync_reservation_to_hubspot(reservation):
    ...
    try:
        ...
        if hasattr(reservation, "hubspot_deal_id"):
            ...
            reservation.save()
            logger.info(
                f"Updated reservation #{reservation.id} with HubSpot deal ID {deal_id}"
            )
