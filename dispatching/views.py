from drivers.models import Driver  # Assuming you have a drivers app
...
def index(request):
    ...
    try:
        selected_date = datetime.strptime(selected_date, '%Y-%m-%d').date() if selected_date else timezone.localdate()
    except ValueError:
...
def all_reservations(request):
    ...
    total_revenue = reservations_query.aggregate(
...
def modify_reservation(request, id):
    reservation = get_object_or_404(
        Reservation.objects.prefetch_related('legs'), 
        uuid=id
    )
    ...
    if request.method == 'POST':
        ...
        if customer_form.is_valid() and reservation_form.is_valid():
            ...
            for i in range(1, 3):  # Support up to 2 legs
                leg_prefix = f'leg_{i}'
                leg_data = {
                    f'{leg_prefix}-pickup_date': request.POST.get(f'{leg_prefix}-pickup_date'),
                    f'{leg_prefix}-pickup_time': request.POST.get(f'{leg_prefix}-pickup_time'),
                    f'{leg_prefix}-pickup_location': request.POST.get(f'{leg_prefix}-pickup_location'),
                    f'{leg_prefix}-dropoff_location': request.POST.get(f'{leg_prefix}-dropoff_location'),
                }
                
                if any(leg_data.values()):
                    leg_instance = reservation.legs.all()[i-1] if reservation.legs.count() >= i else None
                    leg_form = LegForm(leg_data, instance=leg_instance, prefix=leg_prefix)
                    if leg_form.is_valid():
                        leg = leg_form.save(commit=False)
                        leg.reservation = updated_reservation
                        leg.save()

            messages.success(request, f'Reservation {updated_reservation.uuid} updated successfully.')
