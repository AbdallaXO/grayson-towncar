        ...
        if "confirm_sync" in request.POST and not active_sync:
            ...
            try:
                if sync_type in ["reservations", "all"]:
                    results["reservations"] = sync_reservations_with_cancel(
                        start_id, limit, dry_run
                    )

                if not cancel_requested and sync_type in ["customers", "all"]:
                    results["customers"] = sync_customers_with_cancel(
                        start_id, limit, dry_run
                    )
    ...
    for reservation in query:
        ...
        try:
            if not dry_run:
                ...
                existing_deal_id = hubspot_service.find_deal_by_reservation_id(
                    reservation.id
                )
